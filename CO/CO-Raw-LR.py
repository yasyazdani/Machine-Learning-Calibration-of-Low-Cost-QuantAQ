...
def screen_and_prepare():
    ...
    for label, df in csv_sources:
        ...
        # df2["temp2"] = df2["temp"]**2                      # not helpful + temp is in C , it should been converted to kelvin first
        ...
        # df2["co_ewma"] = df2["co_diff"].ewm(span=window_samples).mean() #Exponential moving average # not helpful   # not helpful
        # df2 = pd.get_dummies(df2, columns=["dow"], drop_first=True) #Categorical time features
...
# def plot_residuals_vs_time_in_temp_range(
#     sensor_datasets,
#     start_date,
#     end_date,
#     feature_cols,
...
# ):
#     """
#     For each sensor, fits LinearRegression and plots residuals vs time for points
#     where temp is in [temp_min, temp_max].
#     """
#     os.makedirs(out_dir, exist_ok=True)
#     splits = split_all_sensors(sensor_datasets, start_date, end_date, feature_cols)
#     n = len(splits)
...
#         # concat train+test for prediction and filtering
#         full_df = pd.concat([train_df, test_df]).reset_index(drop=True)
#         temps   = full_df["temp"].values
#         times   = full_df["Time_ISO"].values
#         X_full  = full_df[feature_cols].values
#         y_full  = full_df["target"].values

#         y_pred  = pipe.predict(X_full)
#         resids  = y_full - y_pred

#         # Filter by temperature range
#         mask = (temps >= temp_min) & (temps <= temp_max)
#         filtered_times   = full_df.loc[mask, "Time_ISO"]
#         filtered_resids  = resids[mask]
#         filtered_temps   = temps[mask]

#         ax.scatter(filtered_times, filtered_resids, s=10, c=filtered_temps, cmap="viridis")
#         ax.axhline(0, linestyle='--', color='gray', linewidth=0.8)
#         ax.set_title(f"{label} | {temp_min}≤T≤{temp_max}", fontsize=10)
#         ax.set_ylabel("Residual (ppb)")
#         ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
#         plt.setp(ax.get_xticklabels(), rotation=30, ha='right', fontsize=6)
...
#     cbar = fig.colorbar(
#         plt.cm.ScalarMappable(norm=plt.Normalize(vmin=temp_min, vmax=temp_max),
#         cmap="viridis"),

#         ax=axes.tolist(),
#         label="Temperature (°C)"
#     )
#     fname = f"residuals_vs_time_temp_{temp_min}_{temp_max}_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.png"
#     fpath = os.path.join(out_dir, fname)
#     fig.savefig(fpath, dpi=300, bbox_inches='tight')
#     plt.show()
#     print(f"Saved: {fpath}")
...
# # plot_residuals_vs_time_in_temp_range(
# #     sensor_datasets,
# #     start_date,
# #     end_date,
# #     feature_cols,
...
#     """
#     For each sensor:
#         - Fit LinearRegression (with StandardScaler) on train+test split
#         - Scatter plot: residual (actual - predicted) vs relative humidity (rh)
#     """


#     os.makedirs(out_dir, exist_ok=True)
#     n = len(sensor_datasets)
#     ncols = 2
#     nrows = math.ceil(n / ncols)
#     fig, axes = plt.subplots(nrows, ncols, figsize=(ncols*5, nrows*4), sharex=False)
#     axes = axes.flatten()

#     for ax, (label, df) in zip(axes, sensor_datasets.items()):
#         # Fit model on all data for that sensor
#         X = df[feature_cols].values
#         y = df["target"].values
#         rh = df["rh"].values
...
#         # Scatter plot
#         ax.scatter(rh, resids, s=10, alpha=0.6)
#         ax.axhline(0, linestyle='--', color='gray', linewidth=0.8)
#         ax.set_title(label, fontsize=10)
...
#     fig.tight_layout()
#     fname = os.path.join(out_dir, "residuals_vs_rh.png")
#     fig.savefig(fname, dpi=300, bbox_inches='tight')
#     plt.show()
#     print(f"Saved: {fname}")
# # call it
# # plot_residuals_vs_rh(sensor_datasets, feature_cols)


# # ================== Piece-wised functions ============. => non of them were helpful

# def run_piecewise_temp_lr(
#     sensor_datasets,
#     start_date,
#     end_date,
#     feature_cols_base,
...
# ):
#     """
#     Fits linear regression with a piecewise linear temperature term.
#     Adds two features to all sensor DataFrames BEFORE splitting:
#         temp_below = temp if temp <= temp_break else temp_break
#         temp_above = 0   if temp <= temp_break else temp - temp_break
#     One combined plot for all sensors (like run_lr_all_single).
#     """

#     os.makedirs(out_dir, exist_ok=True)

#     # Add piecewise columns to ALL sensor DataFrames BEFORE splitting
#     for df in sensor_datasets.values():
#         temp = df["temp"]
#         df["temp_below"] = np.where(temp <= temp_break, temp, temp_break)
#         df["temp_above"] = np.where(temp > temp_break, temp - temp_break, 0)

#     # Now define the full feature list
#     feature_cols = feature_cols_base + ["temp_below", "temp_above"]

#     # Now it's safe to split!
#     splits = split_all_sensors(sensor_datasets, start_date, end_date, feature_cols)

#     # COMBINED PLOT (all sensors, like run_lr_all_single)
#     fig, ax = plt.subplots(figsize=(10, 4))
#     # Plot actual (same for all sensors)
#     first_label, (_, test_df, _, _, _, _) = next(iter(splits.items()))
#     ax.plot(test_df['Time_ISO'], test_df['target'], color='black', label='Actual (Picarro)')
...
#         rmse = np.sqrt(mean_squared_error(y_test, y_pred))
#         r2   = r2_score(y_test, y_pred)
#         p2   = pearsonr(y_test, y_pred)[0]**2
#         bias = np.mean(y_pred - y_test)
#         print(f"\n=== Sensor: {label} (Piecewise T≤{temp_break}/{temp_break}+) ===")
#         print(f"Test period | RMSE={rmse:6.2f} ppb | R²={r2:5.3f} | Pearson²={p2:5.3f} | bias={bias:6.2f} ppb")
...
#     ax.set_title(f"Piecewise Linear Temp ≤{temp_break}/>{temp_break}: Actual vs Predicted (All Sensors)", fontsize=11)
#     ax.legend(fontsize=8, ncol=2)
#     ax.set_ylabel('CO (ppb)')
#     ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
#     plt.setp(ax.get_xticklabels(), rotation=30, ha='right', fontsize=6)
#     fname = f"piecewiseT{temp_break}_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.png"
#     fig.savefig(os.path.join(out_dir, fname), dpi=300, bbox_inches='tight')
#     plt.show()

# base_features = ["temp","rh","co_diff","time_days", ] 

# # call it
# # run_piecewise_temp_lr( sensor_datasets, start_date,  end_date, feature_cols_base=base_features,)  # don't include "temp_below"/"temp_above" here temp_break=10, out_dir="figures"

# def plot_piecewise_lr_residuals(
#     sensor_datasets,
#     start_date,
#     end_date,
#     feature_cols_base,
...
# ):
#     """
#     Fit piecewise linear regression (using temp_break) and plot residuals:
#       - over time
#       - vs temperature
#       - vs relative humidity (rh)
#     Saves all plots with the model name in the filename.
#     """

#     os.makedirs(out_dir, exist_ok=True)
#     model_name = f"piecewise_lr_T{temp_break}"

#     # Add piecewise features BEFORE splitting
#     for df in sensor_datasets.values():
#         temp = df["temp"]
#         df["temp_below"] = np.where(temp <= temp_break, temp, temp_break)
#         df["temp_above"] = np.where(temp > temp_break, temp - temp_break, 0)
...
#     # Residuals OVER TIME
#     fig_time, axes_time = plt.subplots(nrows, ncols, figsize=(ncols*5, nrows*3), sharex=False)
#     axes_time = axes_time.flatten()
#     # Residuals VS TEMP
#     fig_temp, axes_temp = plt.subplots(nrows, ncols, figsize=(ncols*5, nrows*3), sharex=False)
#     axes_temp = axes_temp.flatten()
#     # Residuals VS RH
#     fig_rh, axes_rh = plt.subplots(nrows, ncols, figsize=(ncols*5, nrows*3), sharex=False)
#     axes_rh = axes_rh.flatten()

#     for i, (label, (train_df, test_df, X_train, X_test, y_train, y_test)) in enumerate(splits.items()):
#         # Fit model
#         pipe = make_pipeline(StandardScaler(), LinearRegression())
#         pipe.fit(X_train, y_train)

#         # All data for residuals
#         full_df = pd.concat([train_df, test_df]).reset_index(drop=True)
#         X_full = full_df[feature_cols].values
#         y_full = full_df["target"].values
#         pred_full = pipe.predict(X_full)
#         resids = y_full - pred_full

#         # 1) Residual over TIME
#         ax_t = axes_time[i]
#         ax_t.plot(full_df["Time_ISO"], resids, marker=".", linestyle="none", alpha=0.7)
...
#         ax_t.set_ylabel("Residual (ppb)")

#         # 2) Residual vs TEMP
#         ax_temp = axes_temp[i]
#         temps = full_df["temp"].values
#         ax_temp.scatter(temps, resids, s=10, alpha=0.7)
#         ax_temp.axhline(0, linestyle="--", color="gray")
#         ax_temp.set_title(label, fontsize=10)
#         ax_temp.set_xlabel("Temperature (°C)")
#         ax_temp.set_ylabel("Residual (ppb)")

#         # 3) Residual vs RH
#         ax_rh = axes_rh[i]
#         rh = full_df["rh"].values
#         ax_rh.scatter(rh, resids, s=10, alpha=0.7)
#         ax_rh.axhline(0, linestyle="--", color="gray")
#         ax_rh.set_title(label, fontsize=10)
#         ax_rh.set_xlabel("Relative Humidity (%)")
#         ax_rh.set_ylabel("Residual (ppb)")

#     # Remove unused subplots
#     for axes in [axes_time, axes_temp, axes_rh]:
#         for j in range(n, len(axes)):
#             plt.delaxes(axes[j])

#     # SAVE
#     for fig, kind in [
#         (fig_time, "residuals_time"),
#         (fig_temp, "residuals_vs_temp"),
#         (fig_rh, "residuals_vs_rh"),
#     ]:
#         fig.tight_layout()
#         fname = os.path.join(out_dir, f"{kind}_{model_name}_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.png")
#         fig.savefig(fname, dpi=300, bbox_inches="tight")
#         plt.show()
#         print(f"Saved: {fname}")

# #  call it
# # plot_piecewise_lr_residuals(sensor_datasets, start_date, end_date, base_features, temp_break=10, out_dir="figures")

# def run_piecewise3_temp_lr(
#     sensor_datasets,
#     start_date,
#     end_date,
#     feature_cols_base,
...
# ):
#     """
#     Piecewise linear regression with three temperature segments:
#       temp_below1 = temp if temp <= break1 else break1
#       temp_1_2    = temp-break1 if break1 < temp <= break2 else (break2-break1 if temp>break2 else 0)
#       temp_above2 = temp-break2 if temp > break2 else 0
#     """
#     import os
#     os.makedirs(out_dir, exist_ok=True)
...
#     for df in sensor_datasets.values():
#         temp = df["temp"]
#         df["temp_below1"] = np.where(temp <= temp_break1, temp, temp_break1)
#         df["temp_1_2"]    = np.where(temp <= temp_break1, 0,
#                               np.where(temp <= temp_break2, temp-temp_break1, temp_break2-temp_break1))
#         df["temp_above2"] = np.where(temp > temp_break2, temp-temp_break2, 0)

#     feature_cols = feature_cols_base + ["temp_below1", "temp_1_2", "temp_above2"]
...
#         p2   = pearsonr(y_test, y_pred)[0]**2
#         bias = np.mean(y_pred - y_test)
#         print(f"\n=== Sensor: {label} (Piecewise <{temp_break1}, {temp_break1}-{temp_break2}, >{temp_break2}) ===")
#         print(f"Test period | RMSE={rmse:6.2f} ppb | R²={r2:5.3f} | Pearson²={p2:5.3f} | bias={bias:6.2f} ppb")
#         lr = pipe.named_steps['linearregression']
#         for feat, coef in zip(feature_cols, lr.coef_):
...
#     ax.set_title(f"Piecewise Linear Temp: <{temp_break1}, {temp_break1}-{temp_break2}, >{temp_break2} (All Sensors)", fontsize=11)
#     ax.legend(fontsize=8, ncol=2)
#     ax.set_ylabel('CO (ppb)')
#     ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
#     plt.setp(ax.get_xticklabels(), rotation=30, ha='right', fontsize=6)
#     fname = f"piecewise3T_{temp_break1}_{temp_break2}_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.png"
#     fig.savefig(os.path.join(out_dir, fname), dpi=300, bbox_inches='tight')
#     plt.show()

# def plot_piecewise3_lr_residuals(
#     sensor_datasets,
#     start_date,
#     end_date,
#     feature_cols_base,
...
# ):
#     """
#     Fit piecewise linear regression (three segments) and plot residuals:
#       - over time
#       - vs temperature
#       - vs relative humidity (rh)
#     """
#     import os
#     os.makedirs(out_dir, exist_ok=True)
#     model_name = f"piecewise3_lr_T{temp_break1}_{temp_break2}"
...
# # Use your base feature set, do NOT include any temp segment features yet.
# base_features = ["temp", "rh", "co_diff", "time_days","temp2", "temp3"]

# # call them:
# # run_piecewise3_temp_lr(
# #     sensor_datasets,
# #     start_date,
# #     end_date,
# #     feature_cols_base=base_features,
# #     temp_break1=10,        # Lower breakpoint
# #     temp_break2=20,        # Upper breakpoint
# #     out_dir="figures"
# # )
# # plot_piecewise3_lr_residuals(
# #     sensor_datasets,
# #     start_date,
# #     end_date,
# #     base_features,
...
# # differnt arguments for differnt inetrval => Not helpful
# def continuous_multiinterval_model(
#     sensor_datasets: dict,
#     start_date: pd.Timestamp,
#     end_date: pd.Timestamp,
#     feature_sets: dict,
...
# ) -> dict:
#     """
#     Train separate LinearRegression models on disjoint temperature intervals using different feature sets,
#     stitch predictions via intercept shifts, print interval & overall metrics and coefficients, and save plots.
#     """
#     os.makedirs(out_dir, exist_ok=True)
#     bp1, bp2 = breakpoints
#     sensor_preds = {}
...
#     for label, df in sensor_datasets.items():
#         # filter by date and split train/test
#         df = df[(df.Time_ISO >= start_date) & (df.Time_ISO <= end_date)].copy()
#         n_test = int(len(df) * test_frac)
#         train = df.iloc[:-n_test]
#         test  = df.iloc[-n_test:]
#         df['y_pred'] = np.nan

#         # 1) train per-interval models
#         models = {}
#         intervals = {
#             'Below':  train[train['temp'] <= bp1],
#             'Middle': train[(train['temp'] > bp1) & (train['temp'] <= bp2)],
#             'Above':  train[train['temp'] > bp2]
#         }
#         for name, subset in intervals.items():
#             feats = feature_sets.get(name, [])
#             if len(subset) >= min_points and feats:
#                 X = subset[feats].values
#                 y = subset['target'].values
#                 pipe = make_pipeline(StandardScaler(), LinearRegression())
#                 pipe.fit(X, y)
#                 models[name] = (pipe, feats)

#         # 2) compute intercept shifts at breakpoints
#         shifts = {}
#         if 'Below' in models:
#             anchor_pipe, anchor_feats = models['Below']
#             def make_sample(feats, tval):
#                 row = {}
#                 for f in feats:
#                     if f == 'temp':
#                         row[f] = tval
...
#                         row[f] = tval**3
#                     elif f == 'rh*temp':
#                         row[f] = train['rh'].mean() * tval
#                     else:
#                         row[f] = train[f].mean()
#                 return pd.DataFrame([row])[feats]

#             if 'Middle' in models:
#                 mid_pipe, mid_feats = models['Middle']
#                 pa = make_sample(anchor_feats, bp1)
#                 pm = make_sample(mid_feats,    bp1)
#                 shifts['Middle'] = float(anchor_pipe.predict(pa) - mid_pipe.predict(pm))

#             if 'Above' in models:
#                 ah_pipe, ah_feats = models['Above']
#                 pa2 = make_sample(anchor_feats, bp2)
#                 pa3 = make_sample(ah_feats,     bp2)
#                 shifts['Above'] = float(anchor_pipe.predict(pa2) - ah_pipe.predict(pa3))

#         # 3) predict, print metrics & coefficients
#         for name, subset in {
#             'Below':  test[test['temp'] <= bp1],
#             'Middle': test[(test['temp'] > bp1) & (test['temp'] <= bp2)],
#             'Above':  test[test['temp'] > bp2]
#         }.items():
#             if name not in models:
#                 continue
#             pipe, feats = models[name]
#             X_te = subset[feats].values
#             y_te = subset['target'].values
#             y_pr = pipe.predict(X_te)
#             if name in shifts:
#                 y_pr += shifts[name]
#             df.loc[subset.index, 'y_pred'] = y_pr

#             # metrics
#             rmse = np.sqrt(mean_squared_error(y_te, y_pr))
#             r2   = r2_score(y_te, y_pr)
#             bias = np.mean(y_pr - y_te)
#             print(f"{label} | {name} | n={len(y_te)} | RMSE={rmse:.2f} | R²={r2:.2f} | bias={bias:.2f}")
...
#         # overall metrics
#         valid = df.dropna(subset=['y_pred'])
#         rmse_all = np.sqrt(mean_squared_error(valid['target'], valid['y_pred']))
#         r2_all   = r2_score(valid['target'], valid['y_pred'])
#         bias_all = np.mean(valid['y_pred'] - valid['target'])
#         print(f"{label} | Overall | RMSE={rmse_all:.2f} | R²={r2_all:.2f} | bias={bias_all:.2f}\n")

#         sensor_preds[label] = df

#     # 4) plot time series
#     plt.figure(figsize=(12,4))
#     first = next(iter(sensor_preds))
#     plt.plot(sensor_preds[first].Time_ISO, sensor_preds[first].target,
#              color='black', label='Actual', linewidth=2)
#     for lbl, dfp in sensor_preds.items():
#         plt.plot(dfp.Time_ISO, dfp.y_pred, '--', label=lbl)
#     plt.legend(ncol=3, fontsize='small')
#     plt.xlabel('Time'); plt.ylabel('CO (ppb)')
#     plt.tight_layout()
#     plt.savefig(os.path.join(out_dir, 'multiinterval_custom_continuous.png'), dpi=300)
#     plt.show()

#     # 5) plot residuals vs temp, time, rh
#     for kind in ['temp', 'Time_ISO', 'rh']:
#         fig, axes = plt.subplots((len(sensor_preds)+1)//2, 2, figsize=(10,6))
#         axes = axes.flatten()
#         for ax, (lbl, dfp) in zip(axes, sensor_preds.items()):
#             res = dfp['target'] - dfp['y_pred']
#             x = dfp[kind] if kind != 'Time_ISO' else dfp['Time_ISO']
#             ax.scatter(x, res, s=8)
#             ax.axhline(0, linestyle='--', color='gray')
#             ax.set_title(f"{lbl} residual vs {kind}")
#             ax.set_xlabel(kind); ax.set_ylabel('Residual (ppb)')
#         plt.tight_layout()
#         fname = f'residuals_vs_{kind}_custom_continuous.png'
...
# # ────────── Example usage ──────────
# feature_sets = {
#     'Below':  ['temp', 'rh', 'co_diff', 'time_days'],
#     'Middle': ['temp', 'rh', 'co_diff', 'time_days', 'temp2'],
#     'Above':  ['temp', 'rh', 'co_diff', 'time_days', 'temp2', 'temp3', 'rh*temp']
# }

# # call it 
# preds = continuous_multiinterval_model(sensor_datasets,start_date, end_date, feature_sets, breakpoints=[10,20])
